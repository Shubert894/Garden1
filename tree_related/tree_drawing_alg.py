'''
Algorithm that takes a tree structure and computes the x and y 
coordinates of every node, then assign these coordinates to
the original structure.

Main function: bucheim(tree)
'''
...
class DrawTree():
    def __init__(self, tree, parent=None, depth=0, number=1):
        ...
        for i, c in enumerate(tree.children):
            dtn = DrawTree(c, self, depth+1, i+1)
            self.children.append(dtn)
        self.parent = parent
        self.thread = None
        self.mod = 0
        self.ancestor = self
